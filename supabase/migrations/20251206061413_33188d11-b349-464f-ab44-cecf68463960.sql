...
-- ============================================================================
-- Multi-Currency Enhancement - Part 1: Schema Changes Only
-- ============================================================================

-- Add missing dual-currency columns to sales_invoices (no data updates for posted records)
ALTER TABLE sales_invoices 
ADD COLUMN IF NOT EXISTS subtotal_fc NUMERIC(18,2) DEFAULT 0,
ADD COLUMN IF NOT EXISTS subtotal_bc NUMERIC(18,2) DEFAULT 0,
ADD COLUMN IF NOT EXISTS discount_amount_fc NUMERIC(18,2) DEFAULT 0,
ADD COLUMN IF NOT EXISTS discount_amount_bc NUMERIC(18,2) DEFAULT 0,
ADD COLUMN IF NOT EXISTS tax_amount_fc NUMERIC(18,2) DEFAULT 0,
ADD COLUMN IF NOT EXISTS tax_amount_bc NUMERIC(18,2) DEFAULT 0,
ADD COLUMN IF NOT EXISTS total_amount_fc NUMERIC(18,2) DEFAULT 0,
ADD COLUMN IF NOT EXISTS total_amount_bc NUMERIC(18,2) DEFAULT 0,
ADD COLUMN IF NOT EXISTS paid_amount_fc NUMERIC(18,2) DEFAULT 0,
ADD COLUMN IF NOT EXISTS paid_amount_bc NUMERIC(18,2) DEFAULT 0;

-- Only update draft invoices (not posted ones)
UPDATE sales_invoices
SET 
  subtotal_fc = subtotal,
  subtotal_bc = CASE WHEN currency_code = 'YER' OR currency_code IS NULL THEN subtotal ELSE subtotal * COALESCE(exchange_rate, 1) END,
  discount_amount_fc = discount_amount,
  discount_amount_bc = CASE WHEN currency_code = 'YER' OR currency_code IS NULL THEN discount_amount ELSE discount_amount * COALESCE(exchange_rate, 1) END,
  tax_amount_fc = tax_amount,
  tax_amount_bc = CASE WHEN currency_code = 'YER' OR currency_code IS NULL THEN tax_amount ELSE tax_amount * COALESCE(exchange_rate, 1) END,
  total_amount_fc = total_amount,
  total_amount_bc = CASE 
    WHEN currency_code = 'YER' OR currency_code IS NULL THEN total_amount 
    ELSE COALESCE(base_currency_total, total_amount * COALESCE(exchange_rate, 1)) 
  END,
  paid_amount_fc = paid_amount,
  paid_amount_bc = CASE WHEN currency_code = 'YER' OR currency_code IS NULL THEN paid_amount ELSE paid_amount * COALESCE(exchange_rate, 1) END
WHERE status = 'draft' AND (subtotal_fc = 0 OR subtotal_fc IS NULL);

-- ============================================================================
-- Update post_sales_invoice function to support multi-currency
-- ============================================================================
...
BEGIN
  ...
  -- Set currency values - CRITICAL: Force YER to have rate=1
  v_currency_code := COALESCE(v_invoice.currency_code, 'YER');
  v_exchange_rate := CASE WHEN v_currency_code = 'YER' THEN 1.0 ELSE COALESCE(v_invoice.exchange_rate, 1.0) END;
  
  -- Calculate FC/BC totals
  v_total_fc := COALESCE(v_invoice.total_amount_fc, v_invoice.total_amount);
  v_total_bc := CASE 
    WHEN v_currency_code = 'YER' THEN v_total_fc 
    ELSE COALESCE(v_invoice.total_amount_bc, v_total_fc * v_exchange_rate)
  END;
  ...
  v_net_amount_fc := v_total_fc - COALESCE(v_invoice.tax_amount_fc, v_invoice.tax_amount, 0);
  v_net_amount_bc := CASE WHEN v_currency_code = 'YER' THEN v_net_amount_fc ELSE v_net_amount_fc * v_exchange_rate END;
  ...
  INSERT INTO gl_journal_entries (
    ...
    CASE WHEN v_currency_code <> 'YER' THEN ' (' || v_currency_code || ')' ELSE '' END,
    'sales',
    p_invoice_id::TEXT,
    true, false, auth.uid()
  ...
  -- 1. Debit: Cash/Receivables (total amount) - use BC for main debit/credit
  ...
  IF COALESCE(v_invoice.tax_amount, 0) > 0 AND v_mapping_tax.credit_account_id IS NOT NULL THEN
    ...
    INSERT INTO gl_journal_lines (
      ...
      0, CASE WHEN v_currency_code = 'YER' THEN v_invoice.tax_amount ELSE v_invoice.tax_amount * v_exchange_rate END,
      0, COALESCE(v_invoice.tax_amount_fc, v_invoice.tax_amount),
      0, CASE WHEN v_currency_code = 'YER' THEN v_invoice.tax_amount ELSE v_invoice.tax_amount * v_exchange_rate END,
      v_currency_code, v_exchange_rate,
      'ضريبة مخرجات'
    );
  ...
  IF COALESCE(v_invoice.discount_amount, 0) > 0 AND v_mapping_discount.debit_account_id IS NOT NULL THEN
    ...
    INSERT INTO gl_journal_lines (
      ...
      CASE WHEN v_currency_code = 'YER' THEN v_invoice.discount_amount ELSE v_invoice.discount_amount * v_exchange_rate END, 0,
      COALESCE(v_invoice.discount_amount_fc, v_invoice.discount_amount), 0,
      CASE WHEN v_currency_code = 'YER' THEN v_invoice.discount_amount ELSE v_invoice.discount_amount * v_exchange_rate END, 0,
      v_currency_code, v_exchange_rate,
      'خصم مبيعات'
    );
  ...
  UPDATE customers 
  SET 
    balance = COALESCE(balance, 0) + (v_total_bc - CASE WHEN v_currency_code = 'YER' THEN COALESCE(v_invoice.paid_amount, 0) ELSE COALESCE(v_invoice.paid_amount, 0) * v_exchange_rate END),
  ...
  LOOP
    ...
    -- Accumulate COGS (always in YER)
    v_cogs_amount := v_cogs_amount + (v_line.quantity * COALESCE(v_line.cost, 0));
  END LOOP;

  -- Post COGS if any (always in YER since inventory is in YER)
...
All monetary amounts are stored in both foreign currency (FC) and base currency YER (BC).
GL journal lines include dual-currency amounts for proper financial reporting.';
