-- Fix convert_pr_to_po function with all required columns
...
BEGIN
    ...
    -- Get default UOM
    SELECT id INTO v_default_uom FROM uoms LIMIT 1;
    ...
    LOOP
        ...
        INSERT INTO po_items (
            po_id, item_id, uom_id, qty_ordered, price, price_fc, price_bc,
            net_amount_fc, net_amount_bc, line_no
        ) VALUES (
            v_po_id, 
            v_item.product_id, 
            COALESCE(v_item.uom_id, v_default_uom), 
            v_item.requested_qty,
            COALESCE(v_item.estimated_unit_cost_fc, 0),
            COALESCE(v_item.estimated_unit_cost_fc, 0), 
            COALESCE(v_item.estimated_unit_cost_bc, 0),
            COALESCE(v_item.line_total_fc, 0), 
            COALESCE(v_item.line_total_bc, 0), 
            v_line_no
        );
    END LOOP;
